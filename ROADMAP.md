...
| R-24 | First-boot **license acceptance gate** | owner 2026-07-11 | **NOTE: cannot be a D-045 checklist step** — skippable ≠ acceptance. Would be a **separate blocking gate** (mounts before/around the shell, not the skippable first-run overlay). **Revisit only if the licensing / distribution posture demands it.** *(Owner flagged an alternative — strike this and build the gate now — pending owner's call; recorded as parked until then.)* | parked |
