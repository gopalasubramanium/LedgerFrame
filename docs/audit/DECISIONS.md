...
| D-067 | Ask panel | KEEP | SSE streaming; fact-pack shown before answer (trust UX); validated-before-display; ephemeral (D-016); privacy-mode label always visible; P-6. **⊕ READING NOTE — "always visible" reads as ONE LOCALITY STATEMENT AT EVERY MOMENT, not as one paragraph (owner ruling, 2026-07-20, AI-surfaces §17-1).** The rule is unchanged and not weakened; what is settled is *what satisfies it*. Since §15-4 the panel carries a **second** served statement of the same fact — the provenance legend — and the owner found both on screen at once at the 3b walk. Two true statements of one fact read as two claims the reader must reconcile. So the promise is kept by a **handover**: **pre-answer the posture line** (a user must know where a question goes *before* they send it, and nothing else on screen can tell them), **post-answer the legend** (which says where it *actually* went — strictly more than the posture line could promise, since the line describes what is CONFIGURED and the legend reports what HAPPENED). **Never both, never neither** — and the *never neither* arm is the one guarded hardest, because the obvious reading of this ruling ("drop the posture line") leaves the pre-ask state with no locality statement at all, which **would** be a D-067 breach. Guards: `AskPanel.test.tsx` §17-1, four tests, including a **count** that reds at 0 as well as at 2. |
