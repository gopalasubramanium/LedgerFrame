...
> **STATUS: PLAN DRAFTED through §10 (verify-first) + §9 (NEEDS DECISION). §9 is
> UNRESOLVED — awaiting the owner one-pass. No code written. Build does not start
> until §9 has no open blocker, §3b deltas are approved, and no §4 amendment is
> unresolved.** Copied from `TEMPLATE-page-build.md`; every §1–§8 row cites the spec
> it derives from. This is the largest remaining page milestone — **two masters land
> here** (Entity CRUD, D-065; Institution master, D-008) — so §9 is deliberately long.
...
## 9. NEEDS DECISION — ⛔ UNRESOLVED (awaiting owner one-pass)
...
|---|------|-------------------------------|--------------------------------------------|
| **9-1 ⚑** | **Institution master — build + FK re-pointing (D-008).** | The master **does not exist**: `accounts.institution` (`models:130`) and `insurance_policy.insurer` (`models:525`) are **both free `String(120)`**. No institutions table, no `Institution` model, no FK — and **no user-extensible master-with-CRUD exists anywhere** in the codebase (sector = free-text column; tag = JSON list; neither is a table). This is greenfield. | **BUILD it** as the first extensible master: a `institutions` table (id · name, unique-by-name, starts empty) + typed `GET/POST/PATCH/DELETE`; migrate `accounts.institution` **and** `insurance_policy.insurer` to FK (nullable), **seeding the master from the distinct existing free-text values** (Amendment-B fold precedent — values migrate IN, never destroyed). Insurance's client-side typeahead (`Insurance.tsx:131-133`) is **superseded** by the master (delta-note on the accepted Insurance page). |
...
*END OF DRAFT — STOP at §9. Owner rules §9 one-pass; nothing above §9 is resolved by the author.*
