...
|------|-------------------------------|----------|------------------------|
| **remove** | `GET /api/v1/estate/meta` → **deleted** (vocab lives on `/refdata`) | **§9-1** (D-005) | The delta table already carries this `remove` row (`API-CONTRACT.md:73`), NOT yet ✅. `/refdata` already serves all four vocabs with labels (`refdata.py:133-136`). Two sources for one vocab is the drift trap — the Insurance §9-3 precedent (delivered 2026-07-16). Confirm no consumer (frontend unbuilt), delete, regen contract same commit, flip the row to ✅. |
...
| **doc-only** | **API-CONTRACT.md** — flip the `/estate/meta` `remove` row to ✅ delivered | **§9-1** | Same-commit contract regen (freeze rule). |
...
*Everything the specs under-specify, surfaced BEFORE build. PROPOSED resolutions
are for the owner to approve one-pass — **resolve NOTHING here**; do not start
build on any open item.*

| # | Item | Why it blocks / what's needed | Proposed resolution (owner to approve) |
|---|------|-------------------------------|--------------------------------------------|
| **9-1** | **Retire `GET /estate/meta`** (D-005) | The delta row (`API-CONTRACT.md:73`) is unshipped; `/refdata` already serves all four vocabs with labels (`refdata.py:133-136`). Two sources for one vocab is the drift trap. | **Delete `/estate/meta`**; confirm no consumer (frontend unbuilt; grep clean); regen contract same commit; flip the delta row to ✅; discriminate the removal by response **shape** not status (Insurance §9-3 precedent). |
...
| **9-3** | **Money / base-currency affix — N/A** (D-105) | There is **no money field** anywhere on this page (§10-4). Recording the absence keeps it a chosen decision, not a missed standard. | **Record as N/A (chosen):** no `*_display` strings, no base-currency affix — readiness tiles are counts. A guard asserts no money-formatted string renders. |
| **9-4** | **Staleness / confidence (A10) — N/A** | No market inputs; every field is a user record (§10-4). | **Record as N/A (chosen):** no staleness/confidence annotation; the honesty burden here is empty-state reasons + the advice guard, not freshness. |
