...
verdicts, randomized = "green across the sampled seeds").

**⊕ F-10 DELTA DONE 2026-07-23** (`36955a6` + `306cf6e`; records `HEAD`). Four vector classes fixed (A leaked-schema,
B cache pollution, C leaked config — lru_cache + os.environ, committed-data), each fail-first; permanent census guard;
**both verdicts GREEN solo — ordered 2109/15, randomized 2109/15 across 10 pre-declared seeds, 11/11, 0 reds** (§F-10
FOUNDATION → EXECUTED). **The R-54 close gate is now satisfiable.** **OWED / NEXT:** (i) **Phase 3a** — the full
Ask-surface scripted pre-pass, **carrying F-8's page-portfolio pre-pass re-run + dated note**; (ii) then **Phase 3b**
(owner walk) → **CLOSE**. Two-commit pattern honoured (delta commits then this record).

**⊕ SESSION-END STATE (2026-07-23) — the scripted 3a pre-pass session (dedicated fresh session).** Executed
**ONLY** Phase 3a and STOPPED for the owner's 3b walk, as ruled. **DONE this session:** the full isolated
Ask-surface scripted pre-pass on `:8399` (prod dist same-origin, both themes, `.env` hash-verified unchanged,
ports free at end) — **45/45 DOM checks GREEN, 0 non-benign console errors**, 15 screenshots at
`docs/plans/assets/r54-3a-*.png` (looked at). Artifacts + F-8 note shipped as `0198179` (this record cites it). All three tier-1 categories (term+figure · action→"Open Holdings"
· nav→"Open Appearance settings"), the UNROUTABLE honest miss, both reachable posture states (disabled +
no-egress local-not-dark), a deep link followed (panel closes, page lands, gate honored 200), the legend +
disclaimer constants, and the ratified fact forms (human labels, 2dp weights, sums-to-100, "unavailable",
one-derivation pack==canonical) all verified. **F-8's owed rite DISCHARGED** — `page-portfolio.md` dated note
(cites `7824327`) + live proof that absent 1Y metrics render `"—"` not fabricated `0.00%` while a genuine
`0.00 SGD` renders. Full step-by-step results at [§Phase 3a EXECUTED](#phase-3a--scripted-pre-pass-reset--isolated-green-before-the-walk).
**Findings: NONE** (the two camera-vs-DOM gaps — TWR at the 30vh scroll-fold; a pack↔page figure "difference"
that was mock-price time-drift — both resolved to correct-by-design). **No code moved**; contract 141/71
unchanged; no suite/gate run owed. **⛔ STOPPED — 3b is the owner's.** **NEXT:** the **Phase 3b owner
acceptance walk** (his own questions across the three categories following each link · the posture toggles read
live · one unanswerable question judged) → then **CLOSE** (both suite verdicts already satisfiable per F-10).
**NO PUSH.**
...
**Helper resets** `ecb_fx.clear()` + `grounding.reset_rate_limit()` were drafted + reverted at the foundation session
(kept out of the tree so the standalone delta lands atomically, reviewed in isolation) — re-added as the delta's
first commit.

#### ✅ EXECUTED 2026-07-23 — `36955a6` (first pass) + `306cf6e` (Class C + committed-data, found by the sample)

**FOUR vector classes, not three** — the census-by-mechanism corrected the guess and the sample found more. Each
fail-first RED→GREEN:

| Class | Mechanism (traced, not guessed) | Fail-first repro (RED) | Fix |
|---|---|---|---|
| **A** leaked schema | unit tests read `settings.privacy_mode` via `assert_egress_allowed` on the shared DB with no schema fixture — **fails in TRUE isolation** (`no such table`) | `test_ai_health` **alone** | shared-DB **clean-slate** (drop+create before every test, reused sync engine) |
| **B** cache pollution | `fx._CACHE`/`ecb_fx._RATES` etc. reset only in `app_client` → `session`-only cost-FX tests | `--randomly-seed=1` (4 failed, 3/3) | autouse `_reset_process_globals` over `tests/isolation.RESET_REGISTRY` |
| **C** leaked config | `base_currency` (PUT /settings) persists in the **lru_cached Settings AND `os.environ`** (`apply_env` does `os.environ.update`) → a domestic book reads cross-currency | **full-suite seed=1** → `test_fully_covered_book_carries_no_cost_fx_flags` | `reload_settings()` + os.environ snapshot/restore in `_isolate_env_file` |
| **committed data** | a test `commit()`s `privacy_mode=true`; the `session` teardown only rolls back → a later non-fixture reader inherits it | `test_backfill.py → test_ai_health` | clean-slate drop+**creates** (data, not just schema) every test |

- **2a CENSUS** — AST sweep of `app/`: 129 module-level mutable candidates → **14 runtime-mutated** (the registry), **+ `@lru_cache`/`@cache` functions** (`get_settings`) after Class C proved the assignment-sweep alone under-counts. `tests/isolation.RESET_REGISTRY` is the one declaration both the reset fixture and the guard read.
- **2e CENSUS GUARD** — `tests/unit/test_module_state_census.py`: proven **RED on a deliberate specimen** (a new module global AND an unregistered `@lru_cache`), GREEN after; blindness-pinned on known anchors. A new unregistered mutable global reds it → the class can't silently regrow.
- **2d SAMPLING (labelled sampling, not proof; no re-rolls)** — the **full** suite, solo, on `306cf6e`:

  | Run | Result | | Run | Result |
  |---|---|---|---|---|
  | ordered (`-p no:randomly`) | **2109 / 15** | | seed 777 | **2109 / 15** |
  | seed 1 *(known repro)* | **2109 / 15** | | seed 1234 | **2109 / 15** |
  | seed 2 | **2109 / 15** | | seed 31337 | **2109 / 15** |
  | seed 3 | **2109 / 15** | | seed 8675309 | **2109 / 15** |
  | seed 42 | **2109 / 15** | | seed 2796713921 | **2109 / 15** |
  | seed 100 | **2109 / 15** | | | |

  **11/11 green, 0 reds.** Seeds declared before the runs. (2108 → **2109**: +1 = the census guard.)
- **Blast radius (schema baseline / clean-slate)** — the ~15 migration/db tests repoint `LEDGERFRAME_DATA_DIR` to their own `tmp_path`; **132 such + victim tests green post-change**, and the whole suite green across the sample. **No product behaviour changed** — fixture plumbing. **Overhead:** the clean-slate drop+creates every test on a **reused** sync engine (~3% suite time), the robust choice (resets any leak source, not only fixtured ones).
- **2f** two delta commits (`36955a6`, `306cf6e`); both verdicts stated above; **NO PUSH**.

**LESSON (Class A/C, named at the ruling):** *a test that passes because an earlier test leaked schema/data/config
is order-dependent even when nobody wrote it that way* — the ordered green was execution-order luck, the same species
as a seed-lucky corroboration. Mechanised by the census guard + the sampling.
...
owed by Phase 1.**~~ (**already discharged** by delta 5 — `page-settings.md` dated note 2026-07-22.)

#### ✅ EXECUTED 2026-07-23 — the scripted Ask-surface pre-pass (isolated `:8399`, prod dist same-origin)

**Ruled pass run per the 2026-07-23 kickoff. Isolated per the harness recipe:** fresh temp data dir +
demo seed, prod `dist` served same-origin by uvicorn on `:8399`, `.env` snapshot-restore **hash-verified
identical before AND after** (`460a2da…`; DB settings written via the API, never `.env`), owner stack
(`:8321`/`:5173`) untouched (down, their doing), ports torn down and free at the end. Playwright driver
(inside `frontend/`, deleted before staging), **both themes**, DOM-asserted (not camera-trusted), the one
benign prod-dist CSP theme-flash filtered **by name**. **45/45 checks GREEN, 0 non-benign console errors
in either theme.** Screenshots at the standing assets path (`docs/plans/assets/r54-3a-*.png`, 15 shots),
**each looked at** (9 examined directly incl. every distinct surface × both themes; the rest DOM-identical).

**Per-step results (both themes unless noted):**
- **Idle posture line** — `POSTURE_DISABLED` ("Model AI is off — answers use built-in intelligence…")
  renders pre-answer. `r54-3a-ask-idle-{light,dark}`.
- **Tier-1 (a) term-with-figure** — "what is XIRR" → 7 facts: figure rows (Net worth · Unrealised P/L ·
  Today's change · Total return · **XIRR `0.57% (invested; annualised)`** · **TWR `unavailable`**) each with
  a trailing pointer ↗, + the `Help · XIRR & TWR` prose fact; legend **"Built-in intelligence only — no
  model was used."**, disclaimer served. *(Camera-vs-DOM reconciled: the `.lf-ask__facts` section is
  `max-height:30vh; overflow-y:auto`, so the 7th row (TWR) sits at the internal-scroll fold below the
  multi-line Help prose — a live probe confirms it renders `unavailable`, `isVisible:true`. By design, not
  a defect.)* `r54-3a-ask-term-{light,dark}`.
- **Tier-1 (b) action-with-labeled-link** — "how do I add a holding" → **exactly one** `Help · Holdings`
  fact, labeled link line **"↗ Open Holdings"**. `r54-3a-ask-action-{light,dark}`.
- **Tier-1 (c) nav-with-labeled-link** — "how do I change the theme" → **exactly one** `Help · Settings`
  fact, labeled link line **"↗ Open Appearance settings"** (the ruled worked example — routes to the
  Appearance tab, not bare Settings). `r54-3a-ask-nav-{light,dark}`.
- **Honest miss (UNROUTABLE)** — "what is the weather in Paris" → **zero facts**, `REFUSAL_UNROUTABLE`
  ("I can't match that to anything I can answer. Try asking about your holdings, net worth, allocation, or
  how a page works."), never a nearest-match. `r54-3a-ask-miss-{light,dark}`. *(The NO-DATA sibling
  `REFUSAL_NO_FACTS` is not live-reachable on the fully-seeded demo — every routed question resolves to
  data; it stays unit-pinned by `test_tier1_miss_split.py`.)*
- **Posture line across reachable states** — no-egress toggled ON (via the settings API): panel goes
  **local, not dark**, posture line = the R-54-amended `POSTURE_NO_EGRESS` ("No-egress is on — this device
  makes no outbound calls, so answers are built on this device from your data and the app's own
  explanations, with no model narration."), `--closed` treatment applied. `r54-3a-ask-noegress-{light,dark}`.
  *(local_openai/remote need a configured provider — not reachable without touching `.env`; out of scope
  for this instance.)*
- **Deep link followed** — "what is my net worth" → clicked the Net worth pointer → **panel closed, page
  landed at `#/net-worth`** (sidebar highlights Net worth, trend renders), **gate honored** (an AI read on
  the accepted install → **200**). `r54-3a-deeplink-networth`.
- **Fact rendering spot-checks vs ratified forms** — human labels (`Allocation — Equity/Property/…`, F-7
  Q2) · 2dp weights (`Top 5 concentration 96.50%`, `Largest position 82.17%`) · **per-class allocation
  sums to 100.00** (F-2 holds — enum-complete, no 92.1% shortfall) · **"unavailable"** (`GLD price`, `TWR`)
  · **one-derivation law**: at a single instant the AI pack == canonical `portfolio/summary` **exactly**
  (Net worth 772,651.88 · Unrealised P/L 79,851.73 · Total return 11.53% · Today's change 0.00) — an
  earlier apparent difference was **mock-price time-drift between two read times**, not a pack↔page
  mismatch. **Sub-cent** and **U+2212 signed change** forms are **not reachable on the default demo seed**
  (no sub-cent token; markets flat → 0.00 change) — both already ratified at 0a-i on camera (`2e9104e`);
  not re-fabricated here.
- **F-8 page-portfolio rite (folded in)** — absent 1Y metrics (`1Y return` · `1Y volatility` ·
  `Return / volatility` · `Max drawdown (1Y)` · `TWR`) render **`"—"`, never a fabricated `0.00%`**; the
  genuine zero `Today's change` renders **`0.00 SGD`**. `r54-3a-portfolio-keystats-{light,dark}`. **Dated
  note written to `page-portfolio.md`** (the owed rite; cites the F-8 fix `7824327`).

**Findings triaged: NONE.** No data-integrity/routing defect, no polish worth filing (the two
camera-vs-DOM gaps both resolved to correct-by-design). **No code moved** — this session added only the
`page-portfolio.md` F-8 dated note + this record + the screenshots; the F-8 fix/guard already shipped
(`7824327`). No suite/gate run owed (nothing compiled changed). Contract **141/71 unchanged**.
