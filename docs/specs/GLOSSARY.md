...
| **Mark reviewed** (PROPOSED 2026-07-13, page-review ND-11 — ratify at the walk) | Record the current state (net worth, confidence, attention count) as a **dated review snapshot** with an optional **note** + **next-review date** (`ReviewLog`). The **only acknowledgement in v2** (per-signal acknowledge/dismiss is ROADMAP). **[Help]** |
| **Severity** (PROPOSED 2026-07-13, page-review ND-11 — ratify at the walk) | How much attention a review item warrants — served, display-cased values **`Review`** (worth a look) and **`Info`** (a low-priority nudge, never a hard wall) (§12rv1-5). Rendered **verbatim** in a chip carrying a **semantic tone** (§12rv1-4, ND-4 reversal): `Review` → the ratified attention/warning colour, `Info` (and any unknown severity) → neutral. **[Help]** |
