...
| `_REVIEW_SOON_DAYS` | 30 | Estate review due within 30 days (or overdue) — **the attention feed** | One month's notice before a scheduled estate review, mirroring `_INSURANCE_SOON_DAYS`; overdue always flags. The Estate `estate_signals()` feeds the Review attention feed. `app/services/estate.py`. **Ratified 2026-07-16 (page-estate §9-8)** — a same-batch code test pins the served threshold to this value. |
